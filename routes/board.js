...
router.get("/delete", function(request, response){
  ...
  con.query(sql, [request.query.board_id], function(err, result, fields){
    //삭제 성공하면 바로 코멘트도 지우자 
    //사실상 이 작업은 두 개의 세부업무로 이루어진 트랜잭션 상황이다. 
    if(err){
      console.log(err);
    }else{
...
router.get("/detail", function(request, response){
  ...
  con.query(sql, [request.query.board_id], function(err, result, fields){
    if(err){
      ...
      console.log(request.query.board_id);
...
router.get('/list', function(req, res, next) {
  ...
  var con = mysql.createConnection(conStr);
  con.connect();
  var sql =" select b.board_id as board_id, title, writer, regdate, hint, count(comment_id) as cnt";
  sql += " from board b left outer join comment c";
  sql += " on b.board_id = c.board_id";
  sql += " group by b.board_id, title, writer, regdate, hint";
  console.log(sql);
  //list.ejs에 배열로 전달되고 있음 
